...
function channel=new_channel(x1,y1,x2,y2,Nt,Nr,type)
Ncl=max(1,poissrnd(1.9));Nray=20;frequency=73e9;a=randn(1);
channel=Channel(x1,y1,x2,y2,Nt,Nr,Ncl,Nray,frequency,a,type);
end

function H=Channel(x1,y1,x2,y2,Nt,Nr,Ncl,Nray,frequency,a,type)
H=zeros(Nt,Nr);
...
%         H=H+a*L_nlos(d,lambda)^1/2*spatial_feature_map(theta,Nt)*(spatial_feature_map(theta,Nr)');
        H=H+a*L_nlos(d,lambda)^1/2*(Array_response(Nr,Nt,theta,0,pi)')*Array_response(Nr,Nt,theta,0,pi);
    end
end
H=(Nt*Nr/Ncl*Nray)^1/2*H+H_los(d,Nt,Nr,lambda,type);
end
...
h=type*(Nr*Nt)^1/2*exp(1i*rand(1)*2*pi)*L_los(d,lambda)^1/2*(Array_response(Nr,Nt,0,0,pi)')*Array_response(Nr,Nt,0,0,pi);
end
